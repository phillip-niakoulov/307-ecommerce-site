    ...
    const createStatusButton = () => {
        ...
        return (
            <div>
                ...
                <select className={'order-summary'} id={`${details._id}status`} onChange={() => {
                    document.getElementById(`${details._id}save`).hidden = false;
                }} defaultValue={details.order_status?.status}>
                    {Object.values(OrderStatus).map((statusOption) => (
                        <option key={`${details._id}${statusOption['value']}`}
                                value={statusOption['value']}>{statusOption['text']}</option>
                    ))}

                </select>
                <button className={'order-summary'} hidden={true} onClick={async () => {
                    const newStatus = document.getElementById(`${details._id}status`).value;
                    const data = await fetch(
                        `${import.meta.env.VITE_API_BACKEND_URL}/api/orders/${details._id}`,
                        {
                            method: 'PUT',
                    ...
                    if (data.status === 201) {
                        document.getElementById(`${details._id}save`).hidden = true;
                    }
                }} id={`${details._id}save`}>Save
                </button>
            </div>
        );
